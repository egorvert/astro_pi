...
class CameraController(MetricController, picamera.array.PiMotionAnalysis):
  ...
  def analyze(self,a): #functiont that determines whether there is motion by looking at vectors with magnitudes larger than 60
    ...
    if (a>60).sum()>10:
      print("Motion detected! at %s" %(datetime.datetime.now())) #change to light matrix output and save a true or false boolean to the main data file with a datetime
    ...
    with picamera.PiCamera() as camera:
      with CameraController(camera) as data_out:
          ...
          camera.start_recording(
                '/dev/null', format='h264', motion_output=data_out) #records for 60 s V
